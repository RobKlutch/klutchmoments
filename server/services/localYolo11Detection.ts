...
// Real YOLOv11 inference service using persistent Python worker
...
class LocalYOLOv11DetectionService {
  ...
  async initialize(): Promise<boolean> {
    try {
      console.log('🚀 Initializing Local YOLOv11 detection service...');
      ...
      console.log('✅ Local YOLOv11 detection service initialized with persistent worker');
      return true;
    } catch (error) {
      console.error('❌ Failed to initialize Local YOLOv11 service:', error);
      return false;
    }
  }
...
class YOLOv11Worker:
    ...
    def load_model(self):
        """Load YOLOv11 ONNX model or prepare OpenCV fallback."""
        if HAS_ONNX and os.path.exists(self.model_path):
            try:
                providers = ['CPUExecutionProvider']
                self.model = ort.InferenceSession(self.model_path, providers=providers)
                print(f"✅ YOLOv11 ONNX model loaded: {self.model_path}", file=sys.stderr)
                return True
    ...
    def preprocess_image(self, image: np.ndarray, input_size=640):
        """Preprocess image for YOLOv11 ONNX model."""
    ...
    def postprocess_detections(self, outputs, img_width, img_height, scale_ratio, padding):
        """Post-process YOLOv11 ONNX outputs."""
        ...
        # YOLOv11 uses class 0 for person
    ...
    def detect_players(self, image_data_url: str, timestamp_ms: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.model is not None:
                # Real YOLOv11 ONNX inference
                print("🎯 Running YOLOv11 ONNX inference...", file=sys.stderr)
            ...
            response = {
                ...
                "modelType": "YOLOv11-Local" if self.model else "HOG-Fallback"
            }
...
def main():
    import os
    model_path = sys.argv[1] if len(sys.argv) > 1 else "yolo11n.onnx"
    worker = YOLOv11Worker(model_path)
    
    print("🔥 YOLOv11 Worker ready for requests", file=sys.stderr)
...
`;
    ...
    console.log('✅ YOLOv11 worker script created');
  }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.pythonWorker.stderr.on('data', (data: Buffer) => {
          ...
          console.log(`🐍 YOLOv11 Worker: ${message.trim()}`);
          
          if (message.includes('YOLOv11 Worker ready') && !initComplete) {
            initComplete = true;
            resolve();
          }
        });
        ...
        this.pythonWorker.on('error', (error: Error) => {
          console.error('YOLOv11 worker error:', error);
          if (!initComplete) {
            reject(error);
          }
        });
        ...
        this.pythonWorker.on('exit', (code: number) => {
          console.log(`YOLOv11 worker exited with code ${code}`);
        ...
        setTimeout(() => {
          if (!initComplete) {
            reject(new Error('YOLOv11 worker initialization timeout'));
          }
        }, 10000);
  ...
  async detectPlayers(imageDataUrl: string, timestampMs: number): Promise<DetectionResponse> {
    if (!this.isInitialized || !this.pythonWorker) {
      throw new Error('YOLOv11 worker not initialized');
    }
...
export const localYolo11DetectionService = new LocalYOLOv11DetectionService();
