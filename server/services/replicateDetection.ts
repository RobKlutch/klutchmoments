...
import { DetectionResponse, PlayerDetection } from './localYolo11Detection';

// Replicate YOLOv11 GPU detection service
// Uses cloud GPU inference for robust player tracking via Replicate API

class ReplicateDetectionService {
  ...
  constructor() {
    this.workerPath = path.join(process.cwd(), 'replicate_worker.py');
  }

  async initialize(): Promise<boolean> {
    try {
      console.log('🚀 Initializing Replicate YOLOv11 GPU detection...');
      ...
      console.log('✅ Replicate YOLOv11 GPU detection initialized');
      return true;
    } catch (error) {
...
`;
    ...
    console.log('✅ Replicate worker script created');
  }

  private async startWorkerProcess(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        this.pythonWorker = spawn('python3', [this.workerPath], {
          stdio: ['pipe', 'pipe', 'pipe'],
          env: { ...process.env, REPLICATE_API_TOKEN: process.env.REPLICATE_API_TOKEN }
        });
        ...
        this.pythonWorker.stderr.on('data', (data: Buffer) => {
          ...
          console.log(`🐍 Replicate Worker: ${message.trim()}`);
          
          if (message.includes('Replicate YOLOv11 Worker ready') && !initComplete) {
            initComplete = true;
            resolve();
          }
        });
        ...
        this.pythonWorker.stdout.on('data', (data: Buffer) => {
          ...
          for (const response of responses) {
            ...
            try {
              ...
              console.error('Failed to parse Replicate worker response:', parseError);
            }
          }
        });

        this.pythonWorker.on('error', (error: Error) => {
          console.error('Replicate worker error:', error);
          if (!initComplete) {
            reject(error);
          }
        });
        ...
        this.pythonWorker.on('exit', (code: number) => {
          console.log(`Replicate worker exited with code ${code}`);
        ...
        setTimeout(() => {
          if (!initComplete) {
            reject(new Error('Replicate worker initialization timeout'));
          }
        }, 15000); // 15 seconds for cloud API connection
  ...
  async detectPlayers(imageDataUrl: string, timestampMs: number): Promise<DetectionResponse> {
    if (!this.isInitialized || !this.pythonWorker) {
      throw new Error('Replicate worker not initialized');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      // Request timeout - longer for cloud API
      setTimeout(() => {
        if (this.pendingRequests.has(requestId)) {
          this.pendingRequests.delete(requestId);
          reject(new Error('Replicate detection request timeout'));
        }
      }, 90000); // 90 seconds for cloud GPU inference (higher res images need more time)
    });
  }
...
export const replicateDetectionService = new ReplicateDetectionService();
