  ...
  update(detections: DetectedPlayer[], frameWidth?: number, frameHeight?: number): void {
    ...
    if (bestMatch && (bestScore >= Tracker.MIN_MATCH_SCORE || bestCenterDist <= 0.25)) {
      ...
      if (isStickyLock) {
        ...
        console.log(`🎯 STICKY LOCK DETECTED: Using exact server coordinates (centerDist=${bestCenterDist.toFixed(3)}, confidence=${this.confidence.toFixed(3)})`);
        ...
        console.log(`🔄 APPLYING SMOOTHING: alpha=${smoothingAlpha.toFixed(3)} (centerDist=${bestCenterDist.toFixed(3)}, confidence=${this.confidence.toFixed(3)})`);
...
export function useSpotlightTracker(
  ...
  const requestDetection = useCallback(async (
    ...
    const MIN_INTERVAL = 5000; // **CRITICAL FIX**: 5s interval to prevent YOLOv8 overload during playback (matches detectionInterval)
  ...
  const startUnifiedTrackingLoop = useCallback(() => {
    ...
    if (!video || !tracker || !selectionAnchor) {
      console.error('🚫 rVFC: Missing requirements - video:', !!video, 'tracker:', !!tracker, 'anchor:', !!selectionAnchor);
      return;
    }
    ...
    const onVideoFrame = async (now: number, metadata: VideoFrameCallbackMetadata) => {
      ...
      const detectionInterval = video.paused ? 500 : 5000; // CRITICAL FIX: 5s interval to prevent YOLOv8 overload during playback
  ...
  const startVideoTracking = useCallback(() => {
    ...
    if (typeof video.requestVideoFrameCallback === 'function') {
      console.log('✅ Using requestVideoFrameCallback for video-driven tracking');
      ...
      console.log('⚠️ requestVideoFrameCallback not available, falling back to timeupdate + RAF');
  ...
  useEffect(() => {
    ...
    if (selectionAnchor && trackerRef.current) {
      ...
      if (safePlayer && safePlayer.width && safePlayer.height) {
        ...
        console.log('🎯 SEEDED WITH ACTUAL PLAYER BBOX:', safePlayer.id, boxSize);
  ...
  useEffect(() => {
    ...
    console.log('🎬 Player selected - starting tracking system immediately with enhanced debugging');
    ...
    console.log('🔄 Starting tracking loop (works for both internal and external modes)');
    ...
    const autoRebindInterval = setInterval(() => {
      if (selectionAnchor && trackerRef.current) {
        console.log('🔄 PERIODIC AUTO-REBIND CHECK');
        autoRebindToActiveVideo();
      }
    }, 2000);
  ...
  useEffect(() => {
    ...
    const handleSeeked = () => {
      console.log('🎬 Video seeked - resetting detection timing', {
      ...
      if (highlightLockRef.current) {
        highlightLockRef.current.resetPredictionTimebase();
        console.log('🔄 HighlightLock prediction timebase reset after seek');
      }
    };
  ...
  const ingestDetections = useCallback((payload: { players: DetectedPlayer[]; frameWidth: number; frameHeight: number; timestampMs: number }) => {
    ...
    if (!highlightLockRef.current && detections.length > 0 && selectionAnchor) {
      ...
      if (targetDetection) {
        ...
        console.log(`🔒 ID-LOCK ENFORCED: HighlightLock will track player ${selectedPlayerId} via detection matching`);
    ...
    if (highlightLockRef.current) {
      ...
      if (updatedBox) {
        setCurrentBoxState(prev => {
          ...
          if (!prev || prev.x !== newBox.x || prev.y !== newBox.y || prev.width !== newBox.width || prev.height !== newBox.height) {
            console.log('📦 HIGHLIGHTLOCK CURRENTBOX UPDATE:', {
              from: prev ? `${prev.x.toFixed(3)}, ${prev.y.toFixed(3)}` : 'null',
              to: `${newBox.x.toFixed(3)}, ${newBox.y.toFixed(3)}`,
        ...
        console.log('⚠️ HighlightLock lost confidence:', {
          lockState: highlightLockRef.current.currentState,
          confidence: highlightLockRef.current.confidence.toFixed(3),
          selectedPlayerId: selectedPlayerId || 'none',
          totalDetections: detections.length
      ...
      console.log(`🔗 HighlightLock processed ${detections.length} detections, state: ${highlightLockRef.current.currentState}, ID-lock: ${selectedPlayerId || 'none'}`);
    }
  ...
  if (!currentBox) {
    console.error('❌ CRITICAL ERROR: currentBox is null! This should NEVER happen');
  } else {
    console.log('📦 currentBox is NON-NULL:', {
      box: currentBox,
      center: {
  ...
  useEffect(() => {
    ...
    if (video) {
      ...
      const eventListeners = {
        ...
        timeupdate: (e: Event) => {
          console.log('⏰ VIDEO EVENT: TIMEUPDATE fired on tracker video:', {
            component: options.componentName || 'Unknown',
            currentTime: video.currentTime.toFixed(2),
            paused: video.paused,
            srcShort: video.src ? video.src.substring(video.src.lastIndexOf('/') + 1) : 'NO_SOURCE',
          ...
          if (!video.paused && video.currentTime > 0) {
            console.log(`⏰ TIMEUPDATE: Video advancing to ${video.currentTime.toFixed(3)}s - tracking should be active`);
          }
        }
      };
  ...
  const getBoxByIdAtTime = useCallback((selectedPlayerId: string, sampleTime: number) => {
    ...
    if (tracker) {
      ...
      if (currentBox && currentBox.id === selectedPlayerId) {
        ...
        console.log(`🎯 getBoxByIdAtTime: ID-LOCK MATCH for ${selectedPlayerId}:`, {
      ...
      console.log(`🚫 getBoxByIdAtTime: ID MISMATCH for ${selectedPlayerId}:`, {
        selectedPlayerId,
        currentBoxId: currentBox?.id || 'null',
        sampleTime: sampleTime.toFixed(3),
