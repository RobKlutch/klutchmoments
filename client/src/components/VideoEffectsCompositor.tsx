...
export default function VideoEffectsCompositor({
  ...
  // **UNIFIED TRACKING**: Use consolidated tracking hook
  const { currentBox, status: trackerStatus, ingestDetections } = useSpotlightTracker(
    ...
    { 
      effect: effect.name, 
      settings, 
      selectedPlayer: selectedPlayer,  // **ARCHITECT FIX**: Trust sanitized player from workflow
  ...
  useEffect(() => {
    ...
    // **ARCHITECT FIX**: Trust sanitized player - minimal validation only
    if (selectedPlayer && typeof selectedPlayer.id === 'string') {
      console.log('✅ DATA CONSISTENCY CHECK:');
      console.log('  - selectedPlayer ID:', selectedPlayer.id);
      console.log('  - Center coordinates:', { centerX: selectedPlayer.centerX, centerY: selectedPlayer.centerY });
      console.log('  - Canonical coordinates preserved:', !!(selectedPlayer.topLeftX) && !!(selectedPlayer.topLeftY));
      console.log('  - Player bounding box:', { width: selectedPlayer.width, height: selectedPlayer.height });
      
      if (!selectedPlayer.centerX || !selectedPlayer.centerY) {
        console.error('⚠️ CRITICAL: Missing canonical center coordinates in selectedPlayer!');
      }
      if (!selectedPlayer.topLeftX || !selectedPlayer.topLeftY) {
  ...
  const detectPlayersInFrame = useCallback(async (frameBlob: Blob, signal: AbortSignal): Promise<DetectedPlayer[]> => {
    try {
      ...
      // **SPATIAL FIX**: Always send live tracking coordinates for ID-LOCK RECOVERY
      // Backend can re-associate coordinates even when ID is temporarily missing
      if (currentBox) {
        // Compute center from tracking box coordinates
        const centerX = currentBox.x + currentBox.width / 2;
        const centerY = currentBox.y + currentBox.height / 2;
        
        // **ID RESOLUTION**: Try multiple sources for player ID
        // 1. currentBox.id (from ID-locked tracking)
        // 2. selectedPlayer?.id (from initial selection)
        const playerId = (currentBox as any).id || selectedPlayer?.id;
        
        // Always send coordinates, even if ID is missing (backend can re-associate)
        if (playerId) {
          requestBody.selectedPlayerId = playerId;
        }
        
        requestBody.selectedPlayer = {
          id: playerId || 'unknown', // Flag missing IDs so backend can re-associate
          x: currentBox.x,
          y: currentBox.y,
          centerX,
          centerY,
      ...
      } else if (selectedPlayer && selectedPlayer.id) {
        // Fallback to initial selectedPlayer coordinates when no live tracking box
        requestBody.selectedPlayerId = selectedPlayer.id;
        
        if (selectedPlayer.centerX !== undefined || selectedPlayer.x !== undefined) {
          requestBody.selectedPlayer = {
  ...
  useEffect(() => {
    if (videoRef.current && videoFile) {
      ...
      return () => {
        ...
        // The URL will be revoked when the workflow restarts in CreatorDashboard
        // URL.revokeObjectURL(videoUrl);  // REMOVED - causes video to fail in subsequent stages
