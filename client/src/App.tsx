...
import VideoPreviewSurface from "@/components/VideoPreviewSurface";
...
function AppContent() {
  ...
  // Initialize workflow step based on URL params (for post-auth redirect)
  const getInitialStep = (): WorkflowState['step'] => {
    const urlParams = new URLSearchParams(window.location.search);
    const stepParam = urlParams.get('step');
    if (stepParam === 'upload' && user) {
      return 'upload';
    }
    return 'hero';
  };
  ...
  // Update workflow step when user logs in and URL has step param
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const stepParam = urlParams.get('step');
    if (stepParam === 'upload' && user && workflow.step === 'hero') {
      setWorkflow(prev => ({ ...prev, step: 'upload' }));
    }
  }, [user]);
  ...
  const handleFrameCapture = async (frameBlob: Blob, timestamp: number) => {
    try {
      ...
      formData.append('videoId', workflow.videoFile?.name || 'tracking-video');
      ...
      // **COORDINATE FIX**: Server sends x,y as TOP-LEFT (not center!)
      const normalizedPlayers = (result.players || []).map((player: any) => {
        const clamp01 = (val: number) => Math.max(0, Math.min(1, val || 0));
        
        // **CRITICAL**: Server sends x,y as TOP-LEFT coordinates, NOT center!
        // Always use explicit fields when available, fallback assumes x,y are TOP-LEFT
        ...
        const centerX = player.centerX !== undefined ? clamp01(player.centerX) : clamp01(player.x + player.width / 2);
        const centerY = player.centerY !== undefined ? clamp01(player.centerY) : clamp01(player.y + player.height / 2);
        
        return {
          ...player,
          x: topLeftX, // x = top-left X (server standard)
          y: topLeftY, // y = top-left Y (server standard)
